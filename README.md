...
|---------|---------------|-------------|
| **Management** | Managed by Docker | Direct host filesystem |
| **Path** | Docker-managed location | Explicit host path |
...
We use **named volumes with bind mounts** (hybrid approach):
...
volumes:
  mariadb_data:
    ...
    driver_opts:
      type: none
      o: bind
      device: ${MARIADB_DATA_PATH:-/home/mzhivoto/data/mariadb}
```

**Why this hybrid approach:**
- ✅ **Named volume benefits:** Managed by Docker, shows in `docker volume ls`
- ✅ **Bind mount benefits:** Data stored in known location for easy backup
- ✅ **Data persistence:** Survives container deletion (`docker-compose down`)
- ✅ **Easy access:** Can directly access data at `/home/user/data/`
- ✅ **Evaluation friendly:** Evaluator can inspect data location

**Data flow:**
```
Container: /var/lib/mysql → Volume: mariadb_data → Host: /home/user/data/mariadb
Container: /var/www/html → Volume: wp_files → Host: /home/user/data/wordpress
```

**Volume sharing:**
- WordPress writes to `wp_files:/var/www/html` (read-write)
- NGINX mounts same `wp_files:/var/www/html:ro` (read-only)
- Result: NGINX serves files WordPress creates
